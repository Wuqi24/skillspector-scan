...
function Test-SelfSkill {
  ...
  try {
    ...
    if (-not (Test-Path -LiteralPath $scanScript)) { [Console]::Error.WriteLine('[DBG-SELF] no scan.ps1 at ' + $scanScript); return $false }
    ...
    if (-not $SelfDev) {
      foreach ($k in @($SelfHashes.Keys)) {
        ...
        if ((Get-NormalizedFileHash $hp) -ne $SelfHashes[$k]) { [Console]::Error.WriteLine('[DBG-SELF] hash mismatch: ' + $k + ' got=' + (Get-NormalizedFileHash $hp)); return $false }
      }
    }
